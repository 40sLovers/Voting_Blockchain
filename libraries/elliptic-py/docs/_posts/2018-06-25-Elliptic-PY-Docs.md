...
```python
from ellipticpy import SigningKey
...
```python
from ellipticpy import SigningKey, NIST384p
...
```python
from ellipticpy import SigningKey, VerifyingKey, NIST384p
sk = SigningKey.generate(curve=NIST384p)
vk = sk.get_verifying_key()
...
ellipticpy takes a number that represents the data being signed, and returns a
...
fixed-length digest, which is then turned into a number that ellipticpy can sign,
and both sign and verify must use the same approach. The default value is
hashlib.sha256, but if you use NIST256p or a longer curve, you can use
...
bytes of entropy, and then use one of the helper functions in ellipticpy.util to
convert it into an integer in the correct range, and then finally pass it
into `SigningKey.from_secret()`, like this:

```python
from ellipticpy import NIST384p, SigningKey
from ellipticpy.util import randrange_from_seed__trytryagain

def make_key(seed):
  secexp = randrange_from_seed__trytryagain(seed, NIST384p.order)
  return SigningKey.from_secret(secexp, curve=NIST384p)
...
# openssl dgst -ellipticpy-with-sha256 -sign sk.pem -out data.sig data
# openssl dgst -ellipticpy-with-sha256 -verify vk.pem -signature data.sig data
# openssl dgst -ellipticpy-with-sha256 -prverify sk.pem -signature data.sig data

sk.sign(msg, hashfunc=hashlib.sha256, sigencode=ellipticpy.util.sigencode_der)
vk.verify(sig, msg, hashfunc=hashlib.sha256, sigdecode=ellipticpy.util.sigdecode_der)
```
...
entropy (as opposed to `from_secret/from_string/from_der/from_pem`,
...
ellipticpy.util.PRNG utility is handy here: it takes a seed and produces a strong
pseudo-random stream from it:

```python
from ellipticpy.util import PRNG
from ellipticpy import SigningKey
rng1 = PRNG("seed")
...
Likewise, ellipticpy signature generation requires a random number, and each
...
```python
from ellipticpy import VerifyingKey, BadSignatureError
...
```python
from ellipticpy import SigningKey, NIST521p
...
```python
from ellipticpy import NIST192p, SigningKey
from ellipticpy.util import randrange_from_seed__trytryagain

def make_key_from_seed(seed, curve=NIST192p):
    secexp = randrange_from_seed__trytryagain(seed, curve.order)
    return SigningKey.from_secret(secexp, curve)
...
# Development
Make changes to the **master** branch and create a pull request. Make sure you pull the request in a separate branch.
